...
- **P2 — Differentiator + acceptance:** submissions + claim-typed closes with GitHub verification (§2.4); grants (§2.5); the **required pre-landing check (§2.10)** binding merges to a valid frozen submission/grant/revocation state; artifact plane (§2.8); migration/attach (§6); the informational living-spec contradiction check (§2.11); **collaborator #2 onboarded as this slice's acceptance test.**
- **P3 — Coordination + visibility:** advisory work reservations (§2.9) at all grains with pub/sub and the read-mostly coordinator credential (§2.3/§2.10); hosted board (§4); messages (workspace+repo); heartbeats; coordinator-agent notification triage (§4); advisory dead-letter warnings.
