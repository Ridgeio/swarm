...
> **Single self-contained document.** Canonical, consolidated specification for evolving Swarm from a single-machine coordination CLI into a small multi-human cloud service, folded into one reader-facing artifact:
...
> - **Appendix C — Operator UX specification** (onboarding-centered; Kimi K3 design pass, reviewed). Its five Part-I-touching findings are folded into Part I.
>
> Component sources remain in the repo (`docs/design/SWARM-CLOUD-V1.md`, `SWARM-BOARD-V2.md`, `repo-backstop.md`, `SWARM-CLOUD-UX.md`); on conflict this consolidated document is the reading source of truth. Provenance + the multi-model review history are in Part I's *Design & methodology* section.
>
> **Operator gate:** nothing is provisioned (Supabase project, GitHub App, domain) until the owner reviews this document — the bill of materials (Part I §8) and the phased plan (§9). Each phase ships behind its own launch-blocking tests (§10).
...
- **Silent worker-token renewal (resolves the multi-day-fleet gap — UX-review finding).** A ≤1h TTL must not force hourly interactive auth on a fleet that runs for days. The CLI **silently re-mints** a worker's narrow token as it nears expiry, using the *human's* keychain-held refresh credential — this is the human's credential minting a worker token (permitted, §2.6), NOT the agent minting (which stays forbidden), and each re-mint is attenuation-only (≤ the human's rights, scoped to that run/task/epoch). Steady state is zero operator interaction; `swarm whoami` surfaces credential health (`valid · auto-renewing · expires in 41m`); a failed re-mint (revoked membership or a killed token family) refuses the agent's next *authoritative* command with the exact `swarm join <name> --force` remedy while the seat stays read-visible. Because renewal never widens scope, it does not reopen the escalation surface.
...
**Delivery-of-record is the stream, never keystrokes (carries the local SQLite lesson forward — UX-review finding).** A message to an agent is authoritatively delivered by the server event stream (or the agent's poll / prompt-hook peek); terminal injection is an idempotent *latency hint only*, always carrying the message id and deduped on it — the transport skips injection if that id is already visible, and ACP/headless agents receive messages as protocol events with no injection at all. If the cloud design ever elevated push to authoritative, every mid-paste race in the local system would recur at network scale; it must not.

**Redirect carries uptake confirmation (closes the basketball loop — UX-review finding).** A mid-flight redirect (`swarm send --kind redirect`) is marked as requiring uptake: the worker acknowledges with a *revised* scoped plan (the same §2.12 artifact it emits at claim), and until that lands the board shows the lane **redirect-pending**; an unconfirmed redirect ages into the NEEDS-YOU queue and pings the coordinator after a short interval. A pass is never allowed to go uncaught-and-unnoticed.
...
**Designed onboarding states (UX-review findings).** An invitee whose GitHub repo access is not yet provisioned lands in a first-class **access-pending** state (`swarm doctor --watch` continues automatically when the owner grants access), never a dead red. A freshly-created workspace with a single Owner carries a **sole-owner** prompt to invite and promote a second Owner (required from P2, §2.6) rather than erroring later at the acceptance gate. The **invite page is the product's first touch** — it shows whose workspace, what Swarm is in 30 seconds, the repos it coordinates, and the three steps (install → log in → accept), comprehension before commitment; viewing needs no account, accepting needs a verified identity. The complete operator onboarding UX is Appendix C §1.
...
- **No cross-tenant enumeration:** a non-member cannot discover that a workspace exists, its name, or its members through any surface (no public directory); browsing is limited to your own memberships.
- **Delivery idempotency:** an agent message takes effect exactly once — injection carries the message id and is deduped, and a stream/poll re-delivery of the same id is a no-op (0 lost/duplicated per 1,000 sends).
- **Silent renewal, no lockout:** a multi-day fleet stays authoritative with zero operator auth interaction (silent re-mint from the human credential); a revoked membership refuses the agent's next authoritative command and drops it to read-only with the exact remedy, never a silent half-state.
- **Redirect uptake:** a `--kind redirect` not acknowledged with a revised plan within the interval ages into NEEDS-YOU and pings the coordinator — never unconfirmed and invisible.
- **Access-pending is a state, not an error:** an invitee without GitHub access yet lands in a watchable pending state; `swarm doctor --watch` proceeds automatically on grant.
- **Web surface authority parity:** every authority operation on the web home/admin surface requires human credentials exactly as its CLI equivalent (no agent-token path).
...
> Referenced by Part I §2.11 and installed at Part I §9 P0. The drop-in text placed into each collaborative repo's agent-facing docs.
...
> Authored by a Kimi K3 (third model family) design pass over Part I and the real CLI source, reviewed by the Fable author. Its five Part-I-touching findings are folded into Part I (see Part I §2.3/§2.13/§4/§7); this appendix is the UX elaboration, onboarding (§1) the centerpiece. Section numbers below are local to this appendix.
