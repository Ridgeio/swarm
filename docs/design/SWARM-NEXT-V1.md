...
5. **`swarm handoff offer <slug> --to <agent>` → `swarm handoff accept <slug>`** — requires a checkpoint fresher than 60 min (or `--stale-ok`, which labels the brief STALE). Composes an immutable charter under `~/.swarm/briefs/handoff-offers/` with the latest checkpoint verbatim + task row facts + unpushed-branch inventory + transcript_hint. The durable `handoff_offers` row binds source epoch, named parties, charter SHA-256, pickup deadline, pointer message ID, and delivery state. Offer leaves the source lease unchanged. Only the named recipient can accept; acceptance re-hashes the charter and atomically bumps/transfers the epoch. Expiry, digest mismatch, or changed source authority dead-letters/invalidates the offer with no transfer. `status` exposes pending/injected/acked pickup. The older `swarm handoff <slug> --to <agent>` remains compatibility-only and warns that it transfers immediately.
6. **`swarm decision <text> [--task <slug>] [--supersedes <id>]`** — one-line durable decision journal (`DECISION: X BECAUSE y` doctrine). Task-bound decisions require the current fenced owner and record task epoch + authenticated agent-row provenance; only the original authenticated author may supersede a decision at the same scope. Cheap, greppable, survives every context death.
