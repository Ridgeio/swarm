...
- If the selected swarm is **empty** (no agents or "No agents in swarm"), join as **"Lead"** — you're the first agent in this project swarm, so you coordinate the team. As Lead, **open the fleet board right after joining so you and the operator can see the swarm**: `swarm board --tab` opens a live, self-refreshing board in its own cmux tab (NEEDS YOU / TASKS / FLEET / DEBRIS), and `swarm board --graph --open` renders the visual workflow diagram (agents colored by model, tasks by state, handoff edges). Keep the board tab open — it's the operator's window into the fleet.
