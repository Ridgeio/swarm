...
import { AppleScriptTransport, hasSurface } from './applescript-transport.js';
...
export function getTransport(agentType: string, swarmId?: string, agentName?: string): Transport {
  switch (agentType) {
    case 'a2a': return a2a;
    case 'headless':
      // Use AppleScript push if a surface is registered, otherwise inbox-only
      if (swarmId && agentName && hasSurface(swarmId, agentName)) return applescript;
      return headless;
